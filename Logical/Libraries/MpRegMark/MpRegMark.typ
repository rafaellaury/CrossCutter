(*MpRegMarkDetection - Enumerations*)

TYPE
	MpRegMarkControlBufferTypeEnum : 
		( (*Defines how the size of the output buffer is set.*)
		mpREGMARK_BUFFER_NOT_USED := 0, (*The buffer is not used.*)
		mpREGMARK_BUFFER_AUTOMATIC := 1, (*The size of the buffer is calculated automatically, based on the distance of sensor to process and the product lenght.*)
		mpREGMARK_BUFFER_USER_DEFINED := 2 (*The size of the buffer is defined by the user.*)
		);
	MpRegMarkControlFilterTypeEnum : 
		( (*Defines the type of output filter.*)
		mpREGMARK_FILTER_NOT_USED := 0, (*A filter is not used.*)
		mpREGMARK_FILTER_MOVING_AVERAGE := 1 (*A moving average filter is used.*)
		);
	MpRegMarkControlTypeEnum : 
		( (*Defines the used control type.*)
		mpREGMARK_CONTROL_POSITION_ERROR := 0, (*The mark position error is used for control.*)
		mpREGMARK_CONTROL_PRODUCT_LENGTH := 1 (*The actual product length is used for control.*)
		);
...
(*MpRegMarkDetection - Information*)
...
TYPE
	...
	END_STRUCT;
	MpRegMarkStatusIDType : 	STRUCT  (*Contains diagnostic information for the function block.
*)
		ID : MpRegMarkErrorEnum; (*Error code for the mapp component.*)
		Severity : MpComSeveritiesEnum; (*Defines the type of information (success, information, warning, error).*)
		Code : UINT; (*Code for the status ID.*)
	END_STRUCT;
END_TYPE

(*MpRegMarkDetection - SENSOR SIGNAL*)
...
TYPE
	...
	END_STRUCT;
	MpRegMarkSensorEdgeNettimeType : 	STRUCT  (*Parameters for the NetTime based detection.*)
		TimeStamp : MpRegMarkNettimeTimeStampType; (*Parameters for time stamp.*)
	END_STRUCT;
	MpRegMarkNettimeTimeStampType : 	STRUCT  (*Parameters for time stamp.*)
		ChannelMapping : STRING[255]; (*Mapping of the NetTime based timestamp channel.*)
	END_STRUCT;
END_TYPE

(*MpRegMarkDetection - MEASUREMENT*)
...
TYPE
	...
	MpRegMarkIntervalPositionSrcType : 	STRUCT  (*Source of the interval position.*)
		Type : MpRegMarkIntervalPositionSrcEnum := mpREGMARK_INTERVAL_POS_SRC_SET; (*Defines the source of the interval position.*)
		ParID : UINT := 0; (*ParID which is used as position source.*)
	END_STRUCT;
	MpRegMarkIntervalLengthType : 	STRUCT  (*Parameters for the interval length.*)
		Type : MpRegMarkIntervalLengthTypeEnum := mpREGMARK_INTERVAL_LENGTH_USER; (*Defines how the length of the interval is determined.*)
		Value : LREAL := 1000.0; (*User-defined length of the interval.*)
	END_STRUCT;
...
TYPE
	MpRegMarkDetectionType : 	STRUCT  (*Parameters for the functionality of the detection.*)
		...
		ExpectationWindow : MpRegMarkExpectationWindowType; (*Parameters for the expectation window.*)
...
(*MpRegMarkDetection - CONTROL*)

TYPE
	MpRegMarkControlType : 	STRUCT  (*Parameters for the control process.*)
		Type : MpRegMarkControlTypeEnum := mpREGMARK_CONTROL_POSITION_ERROR; (*Defines the used control type.*)
		Buffer : MpRegMarkControlBufferType; (*Defines the output buffer.*)
		Filter : MpRegMarkControlFilterType; (*Defines the output filter.*)
		Limiter : MpRegMarkControlLimiterType; (*Defines the output limiter.*)
	END_STRUCT;
	MpRegMarkControlBufferType : 	STRUCT  (*Defines the output buffer.*)
		Type : MpRegMarkControlBufferTypeEnum := mpREGMARK_BUFFER_NOT_USED; (*Defines how the size of the output buffer is set.*)
		NumberOfProducts : UINT := 2; (*Size of the user defined buffer.*)
		DistanceSensorToProcess : LREAL := 1000.0; (*Distance of the sensor to the process.*)
	END_STRUCT;
	MpRegMarkControlFilterType : 	STRUCT  (*Defines the output filter.*)
		Type : MpRegMarkControlFilterTypeEnum := mpREGMARK_FILTER_NOT_USED; (*Defines the type of output filter.*)
		NumberOfProducts : UINT := 2; (*Number of products for filtering.*)
	END_STRUCT;
	MpRegMarkControlLimiterType : 	STRUCT  (*Defines the output limiter.*)
		Enable : BOOL := FALSE; (*Enables the output limiter.*)
		MaximumValue : LREAL := 1000.0; (*Maximum allowed control value.*)
	END_STRUCT;
END_TYPE

(*MpRegMarkDetection - OUTPUT SIGNALS*)
...
TYPE
	MpRegMarkDtnOutSignalType : 	STRUCT  (*Data points for the output signals*)
		...
		ControlValue : STRING[255]; (*Output relevant for the control process.*)
	END_STRUCT;
END_TYPE

(*MpRegMarkDetection - MAIN CONFIGURATION*)
...
TYPE
	MpRegMarkDetectionConfigType : 	STRUCT  (*Configuration parameters for the registration mark detection.*)
		...
		Control : MpRegMarkControlType; (*Parameters for the control process.*)
